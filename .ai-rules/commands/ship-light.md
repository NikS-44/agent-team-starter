...
**Branch (kickoff — before plan/build):** If the worktree is on a feature branch, check whether that work is **already merged** (e.g. `gh pr list --head "$(git branch --show-current)" --state merged` is non-empty, or the user said the old PR is merged and this is new work). If the branch’s PR is **MERGED** (or the user is clearly starting a new topic after a merge), **do not** implement on that branch. Run `git fetch origin`, `git switch main` (or the repo default), align with the remote (`git pull` or, if local `main` is only behind `origin/main` with nothing to keep locally, `git reset --hard origin/main`), then `git switch -c <user>/<new-topic>`. The new work and its PR go on that fresh branch. If there is an **open, unmerged** PR for the current branch and the spec is the same line of work, you may keep that branch. When in doubt, default to a new branch from updated default. **Do not** treat a branch whose previous PR is already merged as the base for a new, separate PR (wrong review surface and easy to confuse in history).
...
7. If the above passes: commit, push the worktree branch, then open or reuse a **still-open** PR:
   - **`gh pr create`** (or use the existing **open, unmerged** PR number from `gh pr view --json number` if the current branch still has that PR) with a concise title and body that **includes the verification report** and the **local** screenshot paths under `verification/…` (for traceability). No reviewer-generated prose. If this branch’s prior PR is **merged**, you must be on a **new** branch (see **Branch** above), so always **`gh pr create`** for the new work — do not reuse a merged PR.
